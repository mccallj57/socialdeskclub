...
function ImportModal({initialUrl,initialName,onClose,onDone}){
  ...
  const [job,setJob]=useState(null),[selected,setSelected]=useState([]),[rights,setRights]=useState(false),[approve,setApprove]=useState(false),[includeAllAuthors,setIncludeAllAuthors]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState("");
  ...
  async function preview(e){e.preventDefault();await run(async()=>{const out=await api("/import/preview","POST",tab==="feed"?{url,includeAllAuthors}:tab==="google"?{googleDocUrl,name,text}:{name,text,base64});setJob(out);setApprove(false);setRights(false);setSelected(out.candidates.filter(c=>c.status==="new").map(c=>c.index));});}
